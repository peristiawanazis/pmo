...
$titleBlock = new CTitleBlock('Dashboard', 'applet3-48.png', $m, ($m . '.' . $a));
...
//if ($canAuthor) {
	$titleBlock->addCell(('<form action="?m=macroprojects&amp;a=addedit" method="post">' . "\n" 
	                      ...
	                      . $AppUI->_('new dashboard') . '" />'. "\n" . '</form>' . "\n"));
//}
$titleBlock->addBreadcrumb('', 'Dashboard');
$titleBlock->show();
...
$tabBox->add('pending','Delayed');
$tabBox->add('inprogress','Started vs Not Started');
$tabBox->add('progress','Progress Rate');
$tabBox->add('completion', 'Completion');
if(file_exists('./modules/macroprojects/viewfinances.php')) $tabBox->add('viewfinances', 'Finances');
$tabBox->add('summary','Summary');
$tabBox->show();
?>
