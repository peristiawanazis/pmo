<?php /* MACRO_PROJECTS gantt2.php, v 0.1.0 2012/05/30 */
/*
* Copyright (c) 2012 Region Poitou-Charentes (France)
*
* Description:	Declaration page of class CMacroProject.
*
...
global $company_id, $dept_ids, $department, $locale_char_set, $macroproFilter, $macroprojectStatus, $showInactive, $showLabels, $showAllGantt; // $showAllGantt == Gantt with tasks and users
...
$macroprojectStatus = arrayMerge(array('-2' => $AppUI->_('All w/o in progress')), $macroprojectStatus);
$macroproFilter = dPgetParam($_REQUEST, 'macroproFilter', '-1');

if ($macroproFilter == '-2') {
        $filter1[] = ' macroproject_status != 3';
} else if ($macroproFilter != '-1') {
        $filter1[] = ' macroproject_status = ' . $macroproFilter;
}
if ($company_id != 0) {
        $filter1[] = ' macroproject_company = ' . $company_id;
}

if ($showInactive != '1') {
	$filter1[] = ' macroproject_status <> 7';
}
$pjobj = new CMacroProject;
$allowed_macroprojects = $pjobj->getAllowedSQL($AppUI->user_id);
$where = array_merge($filter1, $allowed_macroprojects);
...
$q->addJoin('macroprojects', 'p', 'p.macroproject_id = t.task_macroproject');
$q->addJoin('companies', 'c', 'p.macroproject_company = c.company_id');
$q->addQuery('u.user_username, t.task_name, t.task_start_date, t.task_milestone' 
             ...
             . ', p.macroproject_color_identifier, p.macroproject_name');
...
$tableTitle = (($macroproFilter == '-1') ? $AppUI->_('All Tasks By Users') : $macroprojectStatus[$macroproFilter]);
...
if (is_array($tasks)) {
	...
	foreach ($tasks as $t) {
		if ($nameUser != $t['user_name']) {
			...
			$barTmp->title->SetColor('#' . $t['macroproject_color_identifier']);
			$barTmp->SetFillColor('#' . $t['macroproject_color_identifier']);
			if (is_file(TTF_DIR . 'FreeSansBold.ttf')) {
		...
		//$progress = $p['macroproject_percent_complete'];
		...
		if ($showLabels) {
			$caption .= ($t['macroproject_name'] . ' (' . $t['perc_assignment'] . '%)');
			/*
			$caption .= (($p['macroproject_status']) != 7 ? $AppUI->_('active') : $AppUI->_('inactive'));
			*/
		}
		...
		if ($t['task_milestone'] != 1) {
			...
			$bar->SetFillColor(('#' . $t['macroproject_color_identifier']));
			$bar->SetPattern(BAND_SOLID, ('#' . $t['macroproject_color_identifier']));
