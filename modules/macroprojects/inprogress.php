...
$(document).ready(function(){
		...
		for(i=0;i<projects.length;i++){
		  ...
		  s2[i] = parseInt(projects[x].started);
		  s5[i] = parseInt(projects[x].p_completed);
		  s2a[i] = parseInt(projects[x].not_started);
		  s3[i] = parseInt(tasks[x].started);
		  s3a[i] = parseInt(tasks[x].not_started);
        ...
        plot1 = $.jqplot('chart1', [s2,s2a], {
			...
			title: {
				text : 'Started and Not Started Projects',
				show : true,
				fontSize : 16,
			},
		...
		plot3 = $.jqplot('chart2', [s3,s3a], {
			...
			seriesDefaults:{
				...
				pointLabels: {show: true, formatString: '%d task'}
			},
            axes: {
                yaxis: {
                    renderer: $.jqplot.CategoryAxisRenderer,
			...
			title: {
				text : 'Started or Not Started Tasks',
				show : true,
				fontSize : 16,
			},
            highlighter: { show: true }
		...
		$('.jqplot-series-0').each(function(i,obj){obj.innerHTML += ' started';});
		if(s2a > 0){
		$('.jqplot-series-1').each(function(i,obj){obj.innerHTML += ' not started';});
     	}
     	else { 
     		$('.jqplot-series-1').each(function(i,obj){obj.innerHTML += ' not started';});
     	}
