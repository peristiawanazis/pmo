...
if ($macroproject_id == 0 && $company_id > 0) {
	$row->macroproject_company = $company_id;
}
...
if ($macroproject_id != 0){
	$titleBlock->addBreadcrumb("?m=macroprojects&amp;a=view&amp;macroproject_id=$macroproject_id", $row->macroproject_name, 'applet3-48.png');
	$titleBlock->addBreadcrumb('', 'Edition', 'icons/pencil.gif');
}else $titleBlock->addBreadcrumb('', 'New MacroProject');
$titleBlock->show();
...
if (color) {
	f.macroproject_color_identifier.value = color;
}
...
< ?php //echo projectSelectWithOptGroup($AppUI->user_id, 'import_tasks_from', 'size="1" class="text"', false, $project_id); ?>
		...
		<tr>
			<td colspan="4">
				...
				<textarea name="macroproject_description" cols="50" rows="10" style="wrap:virtual;" class="textarea">
				<?php echo dPformSafe(@$row->macroproject_description);?>
				</textarea>
			</td>
		</tr>
		</table>
