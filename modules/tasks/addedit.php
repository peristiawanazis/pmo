...
if ($canReadProject) {
	...
	$titleBlock->addBreadcrumb("?m=projects&amp;a=view&amp;project_id=$task_project", @$project->project_name, '../modules/projects/images/applet3-48.png');
	$titleBlock->addCrumb('?m=tasks', 'tasks list');
}else $titleBlock->addBreadcrumb('?m=tasks', 'Tasks');
if ($task_id > 0) {
	$titleBlock->addBreadcrumb(('?m=tasks&a=view&task_id=' . $obj->task_id), $obj->task_name, 'applet-48.png');
	$titleBlock->addBreadcrumb('', 'Edition', 'icons/pencil.gif');
} else $titleBlock->addBreadcrumb('', 'New Task');
...
$selected_departments = (($obj->task_departments != '') ? explode(',', $obj->task_departments) 
                         : array());
// <CRPC add>
$project_departments = (($project->project_departments != '') ? explode(',',$project->project_departments) : array());
$selected_departments = array_merge($selected_departments, $project_departments);
