...
$q = new DBQuery;
$q->addQuery("CONCAT(t.task_id, '|'), IF(SIGN(td.dependencies_delay)=0, t.task_name, CONCAT('[', IF(SIGN(td.dependencies_delay)=1, '+', ''), td.dependencies_delay, ']', t.task_name))");
$q->addTable('tasks', 't');
...
<table width="100%" border="0" cellpadding="4" cellspacing="0" class="std">
			...
			<tr>
				...
				<td nowrap="nowrap">
					<input type="radio" name="task_dynamic" id="task_dynamic_on" value="31" <?php if ($obj->task_dynamic > '20') echo "checked"?> />
				</td>
			</tr>
			<tr>
				<td align="right" nowrap="nowrap"><?php echo $AppUI->_('Off');?></td>
				<td id="no_dyn" nowrap="nowrap">
					<input type="radio" name="task_dynamic" id="task_dynamic_off" value="0" <?php if ($obj->task_dynamic == '0' || $obj->task_dynamic == '11') echo "checked"?> />
				</td>
</tr>
<tr>
	...
	<td>
		<input type="checkbox" name="task_dynamic_nodelay" id="task_dynamic_nodelay" value="1" <?php if (($obj->task_dynamic > '10') && ($obj->task_dynamic < 30)) echo 'checked="checked"' ?> />
			...
			<tr>
				...
				<input type="button" class="button" value="&gt;" onclick="javascript:addTaskDependency(document.dependFrm, document.datesFrm)" />
				<input type="text" size="4" value="+0" name="delay" onclick="this.select();"/><?php echo $AppUI->_('days'); ?>
				<input type="button" class="button" value="&lt;" onclick="javascript:removeTaskDependency(document.dependFrm, document.datesFrm)" />
				</td>
			</tr>
		</table>
