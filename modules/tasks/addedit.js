...
function addTaskDependency(form, datesForm, dayStr) {
	if (dayStr==undefined) dayStr = 'd'
	var at = form.all_tasks.length -1;
	var td = form.task_dependencies.length -1;
	//gets value of delay for selected resource
	var delay = parseInt(form.delay.value);
	...
	for (at; at > -1; at--) {
		if (form.all_tasks.options[at].selected && tasks.indexOf("," + form.all_tasks.options[at].value + ",") == -1) {
			...
			tmp=form.all_tasks.options[at].text;
			if (delay > 0)
				tmp = "[+"+delay+dayStr+"]"+tmp;
			if (delay < 0)
				tmp = "["+delay+dayStr+"]"+tmp;
			opt = new Option(tmp, form.all_tasks.options[at].value+"|"+delay);
			opt.ondblclick = function () {
				changeDelay(form, datesForm, delay, dayStr);
			};
	...
	//Enables tracking of dependencies
	document.getElementById('task_dynamic_on').checked="checked";
	document.getElementById('set_task_start_date').checked="checked";
}

function changeDelay(form, datesForm, delay, dayStr) {
	var newDelay=prompt("",delay+dayStr);
	if (newDelay!=null && newDelay!="")
	{
		var at = form.all_tasks.length -1;
		form.delay.value = parseInt(newDelay);
		removeTaskDependency(form, datesForm);
		t = form.task_dependencies.length
		tmp=form.all_tasks.options[at].text;
		if (newDelay > 0)
			tmp = "[+"+newDelay+dayStr+"]"+tmp;
		if (newDelay < 0)
			tmp = "["+newDelay+dayStr+"]"+tmp;
		opt = new Option(tmp, form.all_tasks.options[at].value+"|"+newDelay);
		opt.ondblclick = function () {
			changeDelay(form, datesForm, newDelay, dayStr);
		};
		form.task_dependencies.options[t] = opt
	}
}
...
function calcFinish(f) {
	...
	var dayType = parseFloat(f.day_type.value); //0 or 1
	...
	for (var i = 0; i < k; i++){
		if (!dayType && !isInArray(working_days, e.getDay())) {
