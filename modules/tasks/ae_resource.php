...
if ($task_id == 0) {
	// <RPC MOD> Add no one
	$assigned_perc = array();
	/*$task_parent = dPgetParam($_GET, 'task_parent', 0);
	if ($task_parent) {
		$parent = new CTask();
		$parent->load($task_parent);
		$assigned_users = $parent->getAssignedUsers();
		$assigned_perc = array();
		foreach($assigned_users as $key=>$row)
			$assigned_perc[$key] = array('contact_name' => $users[$key], 'perc_assignment' => $row['perc_assignment']);
	} else {
...
<tr>
	...
	<td valign="top" align="center">
		...
		<br />
		<input type="checkbox" name="send_ical" id="send_ical" value="1" <?php if ($send_ical=="1"){echo 'checked="checked"';}else{ 'checked=""';}?> />
		<?php echo $AppUI->_('Synchronize with the external calendar of assigned user'); ?> 
		</td></tr></table><br />
	</td>
</tr>
</table>
