<?php /* IGANTT  igantt.php, v0.1.2 2009/02/10 */
/*
...
* version 0.1.2
*	Add new method to set the enhanced scale display flag and modify _createHeader private method accordingly
*
*/
...
var $_showHour	= 0;
/*
*	Enhanced scale flag (if set display grey horizontal lines between bars and shadowed every second columns
*/
...
/**
*	User is allowed to change start/end dates
*/
var $_canMove = true;
...
/*
*	Set enhance scale flag
*		@param	enhanced scale flag (if set display horizontal grey lines between task bars and shadowed every second columns)
*/
...
// Initialize display
	...
	$this->clear( "mainTable", "innerHTML");
...
// Create header HTML and bar container HTML
	...
	$this->assign( "headerLine", "innerHTML", $this->_createHeader() );
...
// Generate left and right time limit for each cell
	$cellDef = array();
	$colwidth = NULL;
	$sdate = new CDate( $this->_startScaleDate );
	$edate = new CDate( $this->_endScaleDate );
...
// Create  Gantt bar container
	...
	$styleCtr = array();
	if ( $this->_enhancedScale ) {
		$styleCtr = array( " style=\"border-top:1px solid #ECECEC;\"", " style=\"border-top:1px solid #ECECEC;background-color:#ECECEC;\"");
	} else {
		$styleCtr = array ( "", "");
	}
	$styleIndex = 0;
	foreach ( $cellDef[$this->_cellPeriod] as $cell ) {
		$this->_barContainer .= "\t\t<td class=\"cellc\" width=\"".$cell['colwidth']."\" align=\"center\" valign=\"top\"{$styleCtr[$styleIndex]} >&nbsp;</td>\n";
...
// Generate scale header table
	$scaleHeader = array();
	if ( $this->_showYear )
		$scaleHeader[] = array( 'content' => $cellDef[GANTT_HYEAR] , 'height' => HEIGHT_HYEAR, 'class' => CLASS_HYEAR, 'enhanced' => $this->_enhancedScale && $this->_cellPeriod == GANTT_HYEAR );
	if ( $this->_showMonth )
		$scaleHeader[] = array( 'content' => $cellDef[GANTT_HMONTH], 'height' => HEIGHT_HMONTH, 'class' => CLASS_HMONTH, 'enhanced' => $this->_enhancedScale && $this->_cellPeriod == GANTT_HMONTH );
	if ( $this->_showWeek )
		$scaleHeader[] = array( 'content' => $cellDef[GANTT_HWEEK], 'height' => HEIGHT_HWEEK, 'class' => CLASS_HWEEK, 'enhanced' => $this->_enhancedScale && $this->_cellPeriod == GANTT_HWEEK );
	if ( $this->_showDay )
		$scaleHeader[] = array( 'content' => $cellDef[GANTT_HDAY], 'height' => HEIGHT_HDAY, 'class' => CLASS_HDAY, 'enhanced' => $this->_enhancedScale && $this->_cellPeriod == GANTT_HDAY  );
	if ( $this->_showHour )
		$scaleHeader[] = array( 'content' => $cellDef[GANTT_HHOUR], 'height' => HEIGHT_HHOUR, 'class' => CLASS_HHOUR, 'enhanced' => $this->_enhancedScale && $this->_cellPeriod == GANTT_HHOUR  );
...
// Generate HTML string
	...
	foreach ( $scaleHeader as $sh ) {
		...
		foreach ( $sh['content'] as $cell ) {
			$s .= "\t\t\t<td class=\"".$cell['class']."\" width=\"".$cell['colwidth']."\" align=\"center\" valign=\"middle\" $style>".$cell['value']."</td>\n";
			$style = $sh['enhanced'] && !$style ? "style=\"background-color:#ECECEC;\"" : "" ;
		}
		$s .= "\t\t\t</tr>\n";
		$s .= "\t\t\t</table>\n";
		$s .= "\t\t</td>\n";
...
// Determine the abscisse value by interpolation
	...
	if ( $val > $this->_cellLimit[$i][1] ) {
		...
		$cellLimitDiff=($this->_cellLimit[$i][1]-$this->_cellLimit[$i][0]);
		if($cellLimitDiff == 0)
			$cellLimitDiff = 1;
		$x += $this->_cellWidth*(($val-$this->_cellLimit[$i][0])/$cellLimitDiff) ;
	}
	...
	//	Create div's for bar display depending on bar type
		switch ( $type ) {
			...
			case GANTT_DYNAMIC :
				...
				$drag = 8;
				break;
/*			case GANTT_ACTIVITY :  // PYB : Version originale avec verrouillage des barre si activité à 100%
				if ( $complete >= 100 ) {
					$s .= "\t<div class=\"at\" ></div>\n";
					$drag = 0;
				} else {
				...
				$s .= "\t</div>\n";
				break;*/
			case GANTT_ACTIVITY :  // PYB : Version permettant de déplacer les barres même si l'activité est à 100%
				$s .= "\t<div class=\"ai\" ></div>\n";
				$drag = $canMove ? (($xstart >= $minDate ? 1 : 0 ) + ( $xend <= $maxDate ? 2 : 0 )) : 0;
				if ( $complete > 0 ) {
					$s .= "\t<div class=\"ac\" style=\"width:{$complete}%;\" ></div>\n";
