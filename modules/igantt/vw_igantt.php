<?php /* IGANTT vw_igantt.php v0.1.2 2009/01/25  */
/*
...
* version 0.1.2
*	Bug fix in $users array creation for user assign: array_merge changed to + (union) to avoid key renumbering
*	Add new option for enhanced scale display
*
*/
...
function newTask() {
	if (document.getElementById('undoBtn').style.visibility != 'hidden')
		alert("<?php echo $AppUI->_('You must save or cancel your changes before adding a task'); ?>")
	else
		document.location = "?m=tasks&a=addedit&task_project=<?php echo $project_id; ?>";
}
</script>
...
<tr>
	...
	<td nowrap="nowrap">
		...
		&nbsp;<?php echo $AppUI->_("Enhanced scale"); ?>
		<input type="checkbox" class="text" name="enhancedScale" id="enhancedScale" />
	</td>
	<td nowrap="nowrap">
...
<div id="taskDetails" style="position:absolute;top:0px;width:650px;z-index:1000;display:none;">
	...
	<tr>
		...
		<td class="hilite" id="assign" align="left" nowrap>
			<?php
				/* PYS Aramis specific user list
				echo arraySelectWithOptGroup( $users, 'task_assign', 'class="text" id="task_assign" style="width:130px;"', 0, false, "Assign user" ); 
				// PYS Aramis end mod	*/
				// Standard dP user list
				$users = array( 0 => "(".$AppUI->_("Assign user").")" ) + $users;
...
<div id="graph">
	...
	<?php
	if (getPermission('projects', 'edit', $project_id) && getPermission('tasks', 'add'))
		echo '<input type="button" class="button" value="'.$AppUI->_('New task').'" onClick="javascript:newTask();" />';
	?>
</div>
<div id="warning"></div>
<br />
